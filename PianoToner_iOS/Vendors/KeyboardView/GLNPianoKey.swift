...
public class GLNPianoKey {
    ...
    func noteLayer(keyRect: CGRect) -> CATextLayer {
        ...
        if (noteNumber < 0 || noteNumber > 108) {
            noteLayer.string = " "
            noteLayer.backgroundColor = UIColor.lightGray.cgColor
        } else {
        ...
        noteLayer.alignmentMode = CATextLayerAlignmentMode.center
    ...
    func noteColourFor(midiNumber: Int, alpha: Float) -> UIColor {
        ...
        return UIColor(hue: hue, saturation: 0.7, brightness: 0.40, alpha: CGFloat(alpha))
    }

    func noteStringFor(midiNumber: Int) -> String {
        return PTNote.midinumberToNoteName(midiNote: midiNumber)
